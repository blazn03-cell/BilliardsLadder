  ...
  const getClimbingStrategyMutation = useMutation({
    mutationFn: () =>
      fetch('/api/ai/community-chat', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          question: `What are the best strategies for players to climb the BilliardsLadder effectively? Consider rating gaps, point systems, streak bonuses, and optimal challenge patterns.`
        })
      }).then(res => res.json()),
    onSuccess: (data) => {
