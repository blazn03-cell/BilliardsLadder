  ...
  const getAIInsightsMutation = useMutation({
    mutationFn: () =>
      fetch("/api/ai/community-chat", {
        ...
        body: JSON.stringify({
          question:
            "Analyze the current ladder trends, player activity, and provide insights about the overall state of competition in the BilliardsLadder billiards community.",
        }),
      }).then((res) => res.json()),
    onSuccess: (data) => {
    ...
    <Card className="bg-black/60 backdrop-blur-sm border border-neon-green/20 shadow-felt">
      ...
      <CardContent>
        ...
        <div className="text-center text-sm text-gray-400 mb-4">
          Scan to join BilliardsLadder instantly
        </div>
