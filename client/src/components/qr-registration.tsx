...
const quickRegistrationSchema = z.object({
  ...
  rating: z
    .number()
    .min(200, "Minimum rating is 200")
    .max(800, "Maximum rating is 800")
    .optional(),
...
function GenerateQRDialog() {
  ...
  const downloadQR = () => {
    ...
    link.download = "match play-ladder-registration-qr.png";
  ...
  const shareQR = () => {
    if (navigator.share) {
      navigator.share({
        title: "Join Match play Ladder Billiards",
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px] bg-black/95 border border-green-500/30">
        <DialogHeader>
          ...
          {!qrCodeData ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-400 mb-6">
                Create a QR code that players can scan to quickly register for
                the ladder
              </p>
              <Button
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-900/50 border border-gray-600/30 rounded-lg p-4">
                ...
                <ul className="text-sm text-gray-300 space-y-1">
                  <li>
                    • Print or display this QR code at your pool hall
                  </li>
                  <li>• Players scan with their phone camera</li>
                  <li>
                    • They'll be taken to a mobile-friendly registration form
                  </li>
                  <li>
                    • New registrations appear instantly in your system
                  </li>
                </ul>
              </div>
            </div>
...
function QuickRegistrationForm({ sessionId }: { sessionId: string }) {
  ...
  const registerMutation = useMutation({
    mutationFn: (data: QuickRegistrationData) =>
      apiRequest("POST", `/api/qr-registration/${sessionId}/register`, data),
    onSuccess: () => {
      toast({
        title: "Registration Successful!",
        description:
          "Welcome to Match play Ladder Billiards! Your profile has been created.",
      });
      form.reset();
    },
    onError: (error: any) => {
  ...
  const onSubmit = (data: QuickRegistrationData) => {
  ...
  return (
    <Card className="max-w-md mx-auto bg-black/90 border border-green-500/30">
      <CardHeader className="text-center">
        ...
        <CardTitle className="text-white">Join Match play Ladder!</CardTitle>
        <p className="text-gray-400 text-sm">
          Quick mobile registration - No membership required!
        </p>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <FormItem>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="New Braunfels">
                        New Braunfels
                      </SelectItem>
                      <SelectItem value="San Marcos">
                        San Marcos
                      </SelectItem>
                    </SelectContent>
                  </Select>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-300">
                    Theme/Motto (Optional)
                  </FormLabel>
                  <FormControl>
                    <Input
                      {...field}
                      placeholder="e.g., 'Chalk and hustle'"
                      className="bg-black/50 border-green-500/30 text-white"
                    />
                  </FormControl>
                  <FormMessage />
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <div className="text-xs text-gray-400 mt-1">
                    You can join and play without any membership. Memberships
                    offer benefits like reduced fees.
                  </div>
                  <FormMessage />
                </FormItem>
              )}
...
function RegistrationStats() {
  const { data: stats } = useQuery({
    queryKey: ["/api/qr-registration/stats"],
  });
      ...
      <Card className="bg-black/60 backdrop-blur-sm border border-green-500/30">
        <CardContent className="p-4 text-center">
          <div className="text-2xl font-bold text-green-400">
            {registrationStats.totalQRRegistrations}
          </div>
      ...
      <Card className="bg-black/60 backdrop-blur-sm border border-green-500/30">
        <CardContent className="p-4 text-center">
          <div className="text-2xl font-bold text-yellow-400">
            {registrationStats.activeSession ? "Active" : "Inactive"}
          </div>
      ...
      <Card className="bg-black/60 backdrop-blur-sm border border-green-500/30">
        <CardContent className="p-4 text-center">
          <div className="text-2xl font-bold text-purple-400">
            {registrationStats.recentRegistrations.length}
          </div>
...
function RecentRegistrations() {
  const { data: recentRegistrations = [] } = useQuery<Player[]>({
    queryKey: ["/api/qr-registration/recent"],
  });
  return (
    <Card className="bg-black/60 backdrop-blur-sm border border-green-500/30">
      <CardHeader>
        <CardTitle className="text-white flex items-center">
          <Users className="w-5 h-5 mr-2 text-green-400" /> Recent QR
          Registrations
        </CardTitle>
      </CardHeader>
      <CardContent>
        {recentRegistrations.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500 text-sm mt-1">
              Generate a QR code to start collecting sign-ups
            </p>
          </div>
        ) : (
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400">
            Easy mobile sign-up for new players
          </p>
        </div>
        <GenerateQRDialog />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-black/60 backdrop-blur-sm border border-green-500/30">
          <CardHeader>
            <CardTitle className="text-white flex items-center">
              <QrCode className="w-5 h-5 mr-2 text-green-400" /> How QR
              Registration Works
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">
                    Create a unique QR code for your venue or event
                  </p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">
                    Print or show the QR code at your pool hall entrance
                  </p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">
                    New players scan with their phone camera
                  </p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">
                    Mobile-friendly form, instant ladder entry
                  </p>
                </div>
              </div>
            </div>
