  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          data-testid="button-create-money-game"
          className="bg-emerald-600 hover:bg-emerald-700"
        >
          <DollarSign className="w-4 h-4 mr-2" /> Start Money Game
        </Button>
      </DialogTrigger>
      <DialogContent className="bg-card border-emerald-500/20">
        <DialogHeader>
          <DialogTitle className="text-emerald-400">
            Create Money on the Table Game
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      placeholder="High Entry fee Money Hunt"
...
function MoneyGameCard({ game }: { game: MoneyGame }) {
  ...
  const joinGameMutation = useMutation({
    mutationFn: () =>
      apiRequest(`/api/money-games/${game.id}/join`, { method: "POST" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/money-games"] });
      toast({
        title: "Joined Game",
  ...
  const startGameMutation = useMutation({
    mutationFn: () =>
      apiRequest("POST", `/api/money-games/${game.id}/start`, {}),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/money-games"] });
      toast({
        title: "Game Started",
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "completed":
        return "bg-gray-600/40 text-gray-300 border-gray-500/40";
      default:
        return "bg-gray-600/40 text-gray-300 border-gray-500/40";
    }
  };
      ...
      <CardHeader className="pb-3">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-400">
          <span className="flex items-center gap-1">
            <Target className="w-4 h-4" /> {game.table}
          </span>
          <span className="flex items-center gap-1">
            <Users className="w-4 h-4" /> {game.currentPlayers}/
            {game.maxPlayers}
          </span>
          <span className="flex items-center gap-1">
            <DollarSign className="w-4 h-4" /> ${game.billAmount} each
          </span>
        </div>
      </CardHeader>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">
            Place your bill and land the cue ball on top to win the prize
            pool
          </p>
        </div>
      ...
      <Card className="bg-card/30 border-emerald-500/20">
        <CardHeader>
          <CardTitle className="text-emerald-400 flex items-center gap-2">
            <Target className="w-5 h-5" /> How to Play
          </CardTitle>
        </CardHeader>
          ...
          <div className="grid md:grid-cols-3 gap-4 text-sm">
            <div className="space-y-2">
              ...
              <p className="text-gray-400">
                Lag from the headstring to see who can stop closest to the bill
                on the table.
              </p>
            </div>
            <div className="space-y-2">
              ...
              <p className="text-gray-400">
                Must strike a rail before the cue ball settles on the
                money.
              </p>
            </div>
            <div className="space-y-2">
              ...
              <p className="text-gray-400">
                Everyone adds more money if no one lands squarely on the
                bill.
              </p>
            </div>
          </div>
      ...
      <div>
        <h3 className="text-xl font-semibold text-white mb-4 flex items-center gap-2">
          <Clock className="w-5 h-5 text-emerald-400" /> Active Games (
          {activeGames.length})
        </h3>
          ...
          <Card className="bg-card/30 border-emerald-500/20">
            <CardContent className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Start a new game to get the match play going!
              </p>
            </CardContent>
          </Card>
        ) : (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-emerald-400">
                $
                {Math.round(
                  games.reduce((sum, game) => sum + game.billAmount, 0) /
