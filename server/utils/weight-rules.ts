...
export class WeightRulesSystem {
  ...
  async updateWeightRules(matchResult: {
    ...
    let rule = await (storage as any).getWeightRule(loserId, winnerId);
    ...
    // Save updated rule
    await (storage as any).updateWeightRule(rule);
  ...
  async getWeightMultiplier(challengerId: string, opponentId: string): Promise<number> {
    ...
    const rule = await (storage as any).getWeightRule(challengerId, opponentId);
  ...
  async clearWeightDebt(winnerId: string, loserId: string): Promise<void> {
    const rule = await (storage as any).getWeightRule(winnerId, loserId);
  ...
  async resetConsecutiveLosses(playerId: string, opponentId: string): Promise<void> {
    const rule = await (storage as any).getWeightRule(playerId, opponentId);
