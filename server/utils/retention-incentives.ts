...
export async function updatePlayerStreak(playerId: string, missedMatch: boolean = false): Promise<PlayerStreak> {
  try {
    ...
    const eligibleReward = (RETENTION_INCENTIVES.STREAK_REWARDS as Record<number, {amount: number, description: string} | undefined>)[streak.currentStreak];
    ...
    await storage.updatePlayerStreak(streak.playerId, streak);
    return streak;
  } catch (error) {
...
async function awardStreakBonus(playerId: string, streakCount: number, amount: number): Promise<void> {
  try {
    ...
    wallet.balanceCredits = (wallet.balanceCredits ?? 0) + amount;
    await storage.updateWallet(wallet.userId, { balanceCredits: wallet.balanceCredits });
...
export async function drawWeeklyWinner(week: string): Promise<string | null> {
  ...
  wallet.balanceCredits = (wallet.balanceCredits ?? 0) + miniPrize.prizeAmount;
  await storage.updateWallet(wallet.userId, { balanceCredits: wallet.balanceCredits });
