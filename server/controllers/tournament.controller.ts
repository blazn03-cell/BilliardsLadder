...
/** Returns the next Friday at 5:00 PM local time */
...
export function completeMatchEntry(storage: IStorage, stripe: Stripe) {
  return async (req: Request, res: Response) => {
    try {
      ...
      // Default platform fee reduced from 10% to 7% for operator-hosted venues.
      const revenueSplitPercent = metadata?.revenueSplitPercent || 7;

      // Free tournament: entry fee is $0 but platform adds a prize pot.
      // Lock the payout until next Friday (payday) — no upgrade required.
      const isFreeEntryMatch = (matchEntry.entryFeePerPlayer === 0);
      ...
      // If this is a free-entry tournament match, lock the payout until next Friday.
      // The prize releases automatically on payday — no upgrade required.
      if (isFreeEntryMatch) {
        const payout = await storage.createPayoutDistribution({
          ...
          transferStatus: "locked_until_payday",
          operatorTierAtPayout: "basic_hall",
          revenueSplitAtPayout: revenueSplitPercent,
          payoutMethod: "stripe_transfer",
          notes: `Prize locked until next payday: ${nextPayday.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}. Auto-releases on Friday at 5pm — no upgrade needed.`,
        });
        ...
        return res.json({
          ...
          payout,
          paydayLock: {
            lockedUntil: nextPayday,
            message: `Your prize of $${(teamPayout / 100).toFixed(2)} is locked until next payday (${nextPayday.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })} at 5pm). It releases automatically — no upgrade needed.`,
          },
        });
      }
