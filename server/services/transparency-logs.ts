...
export class TransparencyLogger {
  ...
  async generateMonthlyReport(month: string): Promise<{
    ...
    type Report = { totalRevenue: number; platformEarnings: number; operatorPayouts: number; bonusFundDistributed: number; playerBenefits: number; transactionCount: number };
    return logs.reduce((report: Report, log: any) => {
