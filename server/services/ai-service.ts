...
async function getRecentMatches(limit: number = 5): Promise<string> {
  try {
    ...
    const recent = matches
      .filter((m: Match) => m.status === 'reported')
      .sort((a: Match, b: Match) => new Date(b.reportedAt || '').getTime() - new Date(a.reportedAt || '').getTime())
      .slice(0, limit);

    return recent.map((m: Match) =>
...
const COMMUNITY_PROMPT = `You are the friendly assistant for the BilliardsLadder billiards community.
...
export class AIService {
  ...
  static async answerCommunityQuestion(question: string): Promise<string> {
    ...
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
      ...
      { 
        role: 'user', 
        content: `Answer this question about the BilliardsLadder billiards community: ${question}
