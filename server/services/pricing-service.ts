...
export const COMMISSION_CONFIG = {
  ...
  get SPLIT_PERCENTAGES() {
    const config = getCurrentConfig();
    return {
      BILLIARDS_LADDER: config.splitPercentages.billiardsLadder,
...
export function calculateCommission(amount: number, membershipTier: string = "none"): {
  ...
  const billiardsLadderShare = Math.floor(roundedCommission * (COMMISSION_CONFIG.SPLIT_PERCENTAGES.BILLIARDS_LADDER / 100));
...
export function calculateSavings(tier: string, monthlyMatches: number): {
  ...
  const billiardsLadderMonthlyCost = tierConfig.price;
  const billiardsLadderMatchFee = 600; // $6/match average with new commission structure

  const billiardsLadderTotal = billiardsLadderMonthlyCost + (billiardsLadderMatchFee * monthlyMatches);
  const competitorTotal = competitorMonthlyCost + (competitorMatchFee * monthlyMatches);

  const monthlySavings = competitorTotal - billiardsLadderTotal;
  ...
  return {
    billiardsLadderCost: billiardsLadderTotal,
