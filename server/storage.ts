...
export interface IStorage {
  ...
  getWeeklyMiniPrize(week: string): Promise<any>;
  createWeeklyMiniPrize(prize: any): Promise<any>;
  updateWeeklyMiniPrize(prize: any): Promise<any>;
  createPlayUpIncentive(incentive: any): Promise<any>;

  // Transparency logs
  createTransparencyLog(entry: any): Promise<any>;
  getTransparencyLogsByMonth(month: string): Promise<any[]>;
  getPlayerStreak(playerId: string): Promise<any | undefined>;
  updatePlayerStreak(playerId: string, updates: any): Promise<any>;
  ...
  calculateTeamChallengeStake(challengeType: string, individualFee: number): Promise<number>;
  ...
  getTutoringCredits(): Promise<TutoringCredits[]>;
  getTutoringCredit(id: string): Promise<TutoringCredits | undefined>;
...
export class MemStorage {
  ...
  // Retention incentives (in-memory stubs)
  private weeklyMiniPrizes: Map<string, any> = new Map();
  private playUpIncentives: Map<string, any> = new Map();

  async getWeeklyMiniPrize(week: string): Promise<any> {
    return this.weeklyMiniPrizes.get(week);
  }

  async createWeeklyMiniPrize(prize: any): Promise<any> {
    this.weeklyMiniPrizes.set(prize.week, prize);
    return prize;
  }

  async updateWeeklyMiniPrize(prize: any): Promise<any> {
    this.weeklyMiniPrizes.set(prize.week, prize);
    return prize;
  }

  async createPlayUpIncentive(incentive: any): Promise<any> {
    const id = `${Date.now()}-${Math.random()}`;
    const record = { ...incentive, id, createdAt: new Date() };
    this.playUpIncentives.set(id, record);
    return record;
  }

  // Transparency logs (in-memory stubs)
  private transparencyLogs: Map<string, any> = new Map();

  async createTransparencyLog(entry: any): Promise<any> {
    const id = entry.id ?? `${Date.now()}-${Math.random()}`;
    const record = { ...entry, id };
    this.transparencyLogs.set(id, record);
    return record;
  }

  async getTransparencyLogsByMonth(month: string): Promise<any[]> {
    return Array.from(this.transparencyLogs.values()).filter(
      (log: any) => log.timestamp && String(log.timestamp).startsWith(month)
    );
  }

  private playerStreaks: Map<string, any> = new Map();

  async getPlayerStreak(playerId: string): Promise<any | undefined> {
    return this.playerStreaks.get(playerId);
  }

  async updatePlayerStreak(playerId: string, updates: any): Promise<any> {
    const existing = this.playerStreaks.get(playerId) || { playerId, currentStreak: 0, longestStreak: 0 };
    const updated = { ...existing, ...updates };
    this.playerStreaks.set(playerId, updated);
    return updated;
  }
  ...
  async calculateTeamChallengeStake(challengeType: string, individualFee: number): Promise<number> {
  ...
  async createTeamChallengeWithParticipants(
    ...
    const totalStake = await this.calculateTeamChallengeStake(challengeData.challengeType, challengeData.individualFee);
  ...
  async getTutoringCredits(): Promise<TutoringCredits[]> {
    return Array.from(this.tutoringCredits.values());
  }

  async getTutoringCredit(id: string): Promise<TutoringCredits | undefined> {
  ...
  async getExpiringStakesHolds(hours: number = 24): Promise<StakesHold[]> {
    ...
    return Array.from(this.stakesHolds.values()).filter(hold => {
      ...
      const createdTime = new Date(hold.createdAt ?? Date.now());
  ...
  async createNotificationSettings(insertSettings: InsertNotificationSettings): Promise<NotificationSettings> {
    ...
    const settings = {
      ...
      reminderT2h: (insertSettings as any).reminderT2h ?? true,
      reminderT30m: (insertSettings as any).reminderT30m ?? true,
      windowOpenPing: (insertSettings as any).windowOpenPing ?? true,
      lateClockStarted: (insertSettings as any).lateClockStarted ?? true,
      noShowFeeCharged: (insertSettings as any).noShowFeeCharged ?? true,
      matchResults: (insertSettings as any).matchResults ?? true,
      quietHoursEnabled: (insertSettings as any).quietHoursEnabled ?? false,
    ...
    this.notificationSettings.set(settings.userId, settings as any);
  ...
  async createNotificationDelivery(insertDelivery: InsertNotificationDelivery): Promise<NotificationDelivery> {
    ...
    const delivery: NotificationDelivery = {
      ...
      subject: insertDelivery.subject ?? null,
  ...
  async createDisputeResolution(insertDispute: InsertDisputeResolution): Promise<DisputeResolution> {
    ...
    this.disputeResolutions.set(id, dispute as any);
  ...
  async addDisputeEvidence(id: string, evidenceUrls: string[], evidenceTypes: string[], notes?: string): Promise<DisputeResolution | undefined> {
    ...
    const currentAuditLog = dispute.auditLog ? JSON.parse(dispute.auditLog as string) : [];
  ...
  async createPlayerCooldown(insertCooldown: InsertPlayerCooldown): Promise<PlayerCooldown> {
    ...
    this.playerCooldowns.set(id, cooldown as any);
    return cooldown as any;
  }
  ...
  async getExpiringCooldowns(hours: number = 24): Promise<PlayerCooldown[]> {
    ...
    return Array.from(this.playerCooldowns.values()).filter(cooldown => 
      cooldown.isActive && 
      (cooldown as any).endsAt <= expiryThreshold &&
      (cooldown as any).endsAt > now &&
      !cooldown.liftedAt
    );
  }
  ...
  async createDeviceAttestation(insertAttestation: InsertDeviceAttestation): Promise<DeviceAttestation> {
    ...
    this.deviceAttestations.set(id, attestation as any);
  ...
  async createJob(insertJob: InsertJobQueue): Promise<JobQueue> {
    ...
    const job: JobQueue = {
      ...
      scheduledFor: insertJob.scheduledFor ? new Date(insertJob.scheduledFor as any) : new Date(),
  ...
  async createSystemAlert(insertAlert: InsertSystemAlert): Promise<SystemAlert> {
    ...
    this.systemAlerts.set(id, alert as any);
    return alert as any;
  }
  ...
  async acknowledgeAlert(id: string, acknowledgedBy: string): Promise<SystemAlert | undefined> {
    return this.updateSystemAlert(id, {
      isActive: false,
      metadata: { acknowledgedBy, acknowledgedAt: new Date() }
    } as any);
  }

  async getSystemAlertsByType(alertType: string): Promise<SystemAlert[]> {
    return Array.from(this.systemAlerts.values()).filter(alert => (alert as any).alertName === alertType);
  }

  async getActiveAlerts(): Promise<SystemAlert[]> {
    return Array.from(this.systemAlerts.values()).filter(alert => (alert as any).isAcknowledged === false);
  }
  ...
  async resolveAlert(alertId: string, resolvedBy: string): Promise<SystemAlert | undefined> {
    return this.updateSystemAlert(alertId, {
      isActive: false,
      metadata: { resolvedBy, resolvedAt: new Date() }
    } as any);
  }
...
export class DatabaseStorage implements IStorage {
  ...
  // Cast to any so delegation calls don't error when MemStorage uses different method names
  private memStorage: MemStorage & Record<string, any> = new MemStorage();
  ...
  async updateWallet(id: string, updates: Partial<InsertWallet>): Promise<Wallet | undefined> { return (this.memStorage as any).updateWallet(id, updates); }
  async deleteWallet(id: string): Promise<boolean> { return this.memStorage.deleteWallet(id); }

  // Retention incentives
  async getWeeklyMiniPrize(week: string): Promise<any> { return (this.memStorage as any).getWeeklyMiniPrize(week); }
  async createWeeklyMiniPrize(prize: any): Promise<any> { return (this.memStorage as any).createWeeklyMiniPrize(prize); }
  async updateWeeklyMiniPrize(prize: any): Promise<any> { return (this.memStorage as any).updateWeeklyMiniPrize(prize); }
  async createPlayUpIncentive(incentive: any): Promise<any> { return (this.memStorage as any).createPlayUpIncentive(incentive); }

  // Transparency logs
  async createTransparencyLog(entry: any): Promise<any> { return (this.memStorage as any).createTransparencyLog(entry); }
  async getTransparencyLogsByMonth(month: string): Promise<any[]> { return (this.memStorage as any).getTransparencyLogsByMonth(month); }
  async getPlayerStreak(playerId: string): Promise<any | undefined> { return (this.memStorage as any).getPlayerStreak(playerId); }
  async updatePlayerStreak(playerId: string, updates: any): Promise<any> { return (this.memStorage as any).updatePlayerStreak(playerId, updates); }
  ...
  async getFileShares(fileId: string): Promise<FileShare[]> { return this.memStorage.getFileShares(fileId); }
  ...
  async getPlatformEarnings(id: string): Promise<PlatformEarnings | undefined> { return this.memStorage.getPlatformEarnings(id); }
  ...
  async getMembershipEarnings(id: string): Promise<MembershipEarnings | undefined> { return this.memStorage.getMembershipEarnings(id); }
  ...
  async getNotificationSettings(userId: string): Promise<NotificationSettings | undefined> { return this.memStorage.getNotificationSettings(userId); }
