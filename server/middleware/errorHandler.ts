/**
 * SILENT FAILURE ANTI-PATTERN — DO NOT DO THIS:
 *
 *   catch (err) { res.json([]); }           // ❌ hides real errors
 *   catch (err) { res.json({ total: 0 }); } // ❌ fake success
 *   catch (err) { return null; }            // ❌ swallows exceptions
 *
 * DO THIS INSTEAD:
 *
 *   catch (err) { next(err); }              // ✅ global handler catches it
 *   throw new NotFoundError("...");         // ✅ typed error → proper response
 *   throw new InternalError("...", {cause}) // ✅ logged + consistent shape
 *
 * The global error handler below returns:
 *   { code, message, requestId }
 * for ALL errors — AppError subclasses and unexpected exceptions alike.
 *
 * See server/lib/errors.ts for the full list of typed error classes.
 */
