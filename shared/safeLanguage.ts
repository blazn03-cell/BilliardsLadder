...
// Master map of risky -> safe terms (Billiards Ladder approved terminology).
const rules: Array<{ pattern: RegExp; safe: string }> = [
  ...
  { pattern: make(`${WORD}(action)${NOT_LETTER}(?!.*ladder)`), safe: "match play" }, // Preserve "Billiards Ladder"
